...
### 缺点
不太容易理解，轮子比较少，某些基础组件还不全，目前还是beta版，有bug。
### 声明式编程
...
```
跟写xml写界面类似，只不过改成了kotlin的方式。需要注意的是Compose中的组件并不是按顺序渲染的，而是系统根据组件的优先级去渲染。
```kotlin
Box(){
    Text("111111")
    Text("222222")
...
```
### Scaffold
脚手架布局，描述界面大体的框架，包含topBar，content,bottomBar,floatingActionButtom,drawer,snackbar等。参数说明：
```kotlin
@Composable
fun Scaffold(
    // 基础样式配置
...
![image.png](https://p6-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/56f7934193d446a899faa91049883cb4~tplv-k3u1fbpfcp-watermark.image)
...
**代码->ui/demo/ImageDemo.kt**

Compose提供了三种加载图片的方式，分别用于支持Bitmap，Vector，Painter加载图片，但是最终都是使用Painter的方式来加载的图片。
### 加载Bitmap
参数说明
```kotlin
@Composable
...
```
How to 加载资源和drawable🌰
```kotlin
Image(painter = painterResource(id = R.drawable.logo), contentDescription = null)
Image(painter = DrawablePainter(drawable) , contentDescription = null)
```
### Coil加载图片
需要导入库
```kotlin
...
### Android Canvas
Compose 支持使用Android Canvas来绘制，举个🌰
```kotlin
@Composable
...
**代码->ui/demo/CustomLayout.kt**

Compose支持用户自定义布局，以及自定义布局修饰符。自定义布局的绘制流程同样是measure->layout->draw。只不过Compose更关心measure和layout这两个过程。
...
### 自定义布局修饰符
布局修饰符即上面提到的Modifier，Compose支持扩展Modifier类，从而实现自己的布局修饰符，举个扩展设置PaddingHorizontal的🌰吧
```kotlin
fun Modifier.paddingHorizontal(padding: Dp) = layout { measurable, constraints ->
    // 这里执行measure，拿到布局支持最大的宽高
...
# 总结
将Compose基本上所有的组件尝试了一遍之后，发现确实能够减少开发的代码量，因为少了各种xml，并且没有了原来那种手动写代码更新视图的繁琐流程，代码量预计少了40%，估计包的大小也能缩水一大半，另外，使用Compose也更容易将复杂的UI界面拆分成一个一个组件，从而提高UI代码的可复用性。

最让人惊讶的是Compose自带就支持了主题的无缝切换，相比于原来切主题的方式，简直妙到家了。
